...
try:
    rnn.load_state_dict(torch.load("cnn_rnn_vector_model_local.pth", map_location=device))
...
def generate_batch(batch_size, p=0.5, kappa=5.0, q=0.0):
    ...
    target_angles = torch.zeros(batch_size) #torch.linspace(0,torch.pi,batch_size) #torch.rand(batch_size) * 2 * torch.pi
    target_colors = torch.zeros(batch_size) #torch.rand(batch_size) * 2 * torch.pi
    # Generate distractor angles
    distractor_angles = torch.linspace(0,torch.pi,batch_size) #torch.zeros(batch_size) torch.zeros(batch_size) #torch.rand(batch_size) * 2 * torch.pi
    distractor_colors = torch.zeros(batch_size) #torch.rand(batch_size) * 2 * torch.pi
...
# %%
plt.plot(delta_theta,torch.cosine_similarity(mean_vector, ground_truth).cpu(),'o')
...
for i, drv in enumerate(dim_reduced_voltages):
    axes.plot(*drv.T, label = i+1, color=plt.cm.RdYlBu(i/batch_size))
    axes.scatter(*drv[0,:,None], marker='o', color=plt.cm.RdYlBu(i/batch_size))
    axes.scatter(*drv[-1,:,None], marker='x', color=plt.cm.RdYlBu(i/batch_size))
axes.legend()
fig.suptitle('PCA of trajectories. Response period starts at o and ends at x')
#fig.savefig('vector_pca_reprs.png')
# %%

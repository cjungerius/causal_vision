...
def generate_batch(batch_size):
    ...
    def generate_gabor_features(angles, colors, model, device, preprocess):
        ...
        gabors[:,1,:,:] = 37 * torch.cos(colors)
        gabors[:,2,:,:] = 37 * torch.sin(colors)        
    ...
    features = generate_gabor_features(labels[:,0], labels[:,1], model, device, preprocess)
...
# %%
y_train_multi = np.array([np.cos(y_train[:,0].cpu()), np.sin(y_train[:,1].cpu()), np.cos(y_train[:,1].cpu()), np.sin(y_train[:,1].cpu())]).T
y_test_multi = np.array([np.cos(y_test[:,0].cpu()), np.sin(y_test[:,1].cpu()), np.cos(y_test[:,1].cpu()), np.sin(y_test[:,1].cpu())]).T
...
ax[0].scatter(y_test[:,0], angle_hat_svm, alpha=.3)
ax[0].scatter(y_test[:,0], angle_hat_nn, alpha=.3, c="orange")
ax[0].set(ylabel="Predicted angle", xlabel="Test angle")

ax[1].scatter(y_test[:,1], color_hat_svm, alpha=.3)
ax[1].scatter(y_test[:,1], color_hat_nn, alpha=.3, c="orange")
